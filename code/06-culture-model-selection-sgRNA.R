...
#     using sgRNA as the primary predictor
...
dat.full <- subset(dat, !is.na(pos_inf) & pos_sg_combined %in% c(0, 1))
dat.culture <- subset(dat, pos_sg_combined == 1 & !is.na(pos_inf) & val_sg_combined != -9)
...
# Set TG indicator based on sgRNA protocol: ordered by decreasing RNA quantity
dat.culture$tg_idx[dat.culture$tg_sg %in% c("N")] <- 1
dat.culture$tg_idx[dat.culture$tg_sg %in% c("E")] <- 2
dat.culture$tg_idx[is.na(dat.culture$tg_sg)] <- 2
dat.culture$tg_idx[dat.culture$tg_sg %in% c("ORF7")] <- 3

dat.culture$val_total[is.na(dat.culture$val_total)] <- -999



# Evaluate RNA negative samples ------------------------------------------------

dat.neg <- subset(dat, val_sg_combined == -9 & !is.na(pos_inf))

# total RNA 
TN_T_neg <- nrow(subset(dat.neg, pos_inf == 0))
FN_T_neg <- nrow(subset(dat.neg, pos_inf == 1))
...
model.culture.xval <- cmdstan_model('./code/stan-model-culture-xval-sg.stan')
...
comp_loo_best <- loo_compare(x = list("c1" = loo_c1,
                                      "c9.1" = loo_c9.1, 
                                      "c10.1" = loo_c10.1))
write.csv(comp_loo_best, file = "./convert-units/manuscript/outputs/EA-table-culture-sgRNA-inf-key-model-comparison-loo.csv",
          row.names = TRUE)
...
# Save raw version
write.csv(tblS4, file = "./outputs/tables/tblS5-culture-sgRNA-model-selection-inf-raw.csv",
          row.names = FALSE)
...
tblS4$elpd <- round(tblS4$elpd, digits = 2)
tblS4$elpd_se <- round(tblS4$elpd_se, digits = 2)
tblS4$elpd_diff_best <- round(tblS4$elpd_diff_best, digits = 2)
tblS4$elpd_diff_best_se <- round(tblS4$elpd_diff_best_se, digits = 2)
...
colnames(tblS4)[colnames(tblS4) == "percent_pos"] <- "% correctly predicted (positive, test)"
colnames(tblS4)[colnames(tblS4) == "percent_neg"] <- "% correctly predicted (negative, test)"
...
write.csv(tblS4, file = "./outputs/tables/tblS5-culture-sgRNA-model-selection-inf-formatted.csv",
          row.names = FALSE)
