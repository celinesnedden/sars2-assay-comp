...
data {
  ...
  int<lower=0> N_new; // number of new predictions
  array[N_new] real t_val_new; // primary predictor, total RNA value
  array[N_new] real dose_new; // log 10 pfu
  array[N_new] int dpi_new; // day post infection
  array[N_new] int sp_new; // species 
  array[N_new] int tg_new; // target gene
...
generated quantities {
  ...
  for (ii in 1:N) {
    ...
    // Posterior predictions on sgRNA value, when article hierarchy unknown
  ...
  // Predicting detectability and values of unknown sgRNA values
  array[N_new] real prob_pos_new; 
  array[N_new] real pred_val_new;
  
    for (ii in 1:N_new) {
      prob_pos_new[ii] = bernoulli_logit_rng(gamma + 
                                             deltaT * t_val_new[ii] +
                                             deltaDOSE * dose_new[ii]  +
                                             deltaTG[tg_new[ii]] +
                                             deltaSP[sp_new[ii]]);
    
    
      pred_val_new[ii] = normal_rng(alpha + 
                                    betaT * t_val_new[ii] + 
                                    betaTG[tg_new[ii]] +
                                    betaSP[sp_new[ii]] + 
                                    betaDPI[dpi_new[ii]] + 
                                    betaDOSE * dose_new[ii],
                                    sigma[discrete_range_rng(1, N_article)]);
